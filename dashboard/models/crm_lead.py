...
class CrmLead(models.Model):
    ...
    @api.model
    def get_tiles_data(self,filter_type):
        print("filter_state", filter_type)
        today = fields.Date.today()
        print("today", today)
        if filter_type == 'year':
            print('year')
            start_date = today.replace(month=1, day=1)
            print('year start_date', start_date)
        elif filter_type == 'daily':
            start_date = today
            print('daily', start_date)

        elif filter_type == 'month':
            start_date = today.replace(day=1)
            print('month start_date', start_date)

        elif filter_type == 'week':
            start_date = today - timedelta(today.weekday())
            print("daily start_date", start_date)

        else:
            quarter_of_the_year = (today.month - 1) // 3 + 1
            print('quarter_of_the_year', quarter_of_the_year)
            quarter_month = (quarter_of_the_year - 1) * 3 + 1
            print("month", quarter_month)
            print('quarter_month', quarter_month)
            start_date = today.replace(month=quarter_month, day=1)
            print('quarter start_date', start_date)
        ...
        if manager_grp:
            ...
            leads = self.search([('company_id', '=', company_id.id),
                                 ('user_id', '=', user_id), ('create_date', '>=', start_date)])
        print("leads", leads)
        ...
        if manager_grp:
            invoiced = self.env['account.move'].search(
                [('state', '=', 'posted'), ('move_type', '=', 'out_invoice'),
                 ('company_id', '=', company_id.id), ('create_date', '>=', start_date)])
        ...
        if manager_grp:
            won_oppurtunity = self.search(
                [('won_status', '=', 'won'), ('company_id', '=', company_id.id), ('create_date', '>=', start_date)])
    ...
    @api.model
    def get_lost_data(self,filter_type):
    ...
    @api.model
    def get_activity_data(self,filter_type):
        print("activity_data")
        today = fields.Date.today()
        print("today", today)
        if filter_type == 'year':
        ...
        if manager:
            activ = self.search([('activity_type_id', 'in', activities.ids), ('company_id', '=', company_id.id),('create_date', '>=', start_date)])
            print("activ", activ)

        else:
            activ = self.search([('activity_type_id', '=', activities.ids), ('user_id', '=', user_id.id),('create_date', '>=', start_date)])
    ...
    @api.model
    def get_medium_data(self,filter_type):
        ...
        for medium in mediums:
            print(medium.name)
        if manager:
            lead_medium = self.search([('medium_id', 'in', mediums.ids),('create_date', '>=', start_date), ('company_id', '=', company_id.id)])
        else:
            lead_medium = self.search(
                [('medium_id', 'in', mediums.ids), ('company_id', '=', company_id.id),('create_date', '>=', start_date), ('user_id', '=', user_id.id)])
        ...
        return [
            ...
            {'medium': 'Google Adwords',
             'count': len(lead_medium.filtered(lambda x: x.medium_id.name == 'Google Adwords'))},
    ...
    @api.model
    def get_campaign_data(self,filter_type):
        print("campaign_date", filter_type)
        print("filter_state", filter_type)
        today = fields.Date.today()
        print("today", today)
        if filter_type == 'year':
        ...
        if manager:
            lead_campaign = self.search([('campaign_id', 'in', campaigns.ids), ('company_id', '=', company_id.id),('create_date','>=',start_date)])
            print("lead_campaign", lead_campaign)
        else:
            lead_campaign = self.search([('campaign_id', 'in', campaigns.ids), ('company_id', '=', company_id.id),
                                         ('user_id', '=', user_id.id),('create_date','>=',start_date)])
        return [
            {'campaign': 'Sale', 'count': len(lead_campaign.filtered(lambda x: x.campaign_id.name == 'Sale'))},
            {'campaign': 'Christmas Special',
             'count': len(lead_campaign.filtered(lambda x: x.campaign_id.name == 'Christmas Special'))},
            {'campaign': 'Email Campaign - Services',
             'count': len(lead_campaign.filtered(lambda x: x.campaign_id.name == 'Email Campaign - Services'))},
            {'campaign': 'Email Campaign - Products',
             'count': len(lead_campaign.filtered(lambda x: x.campaign_id.name == 'Email Campaign - Products'))},
        ]
    ...
    def get_lead_month_data(self):
        ...
        if manager:
            months = self.search([("type", '=', 'lead')])
            print("months", months)
        else:
            months = self.search(
                [('company_id', '=', company_id.id), ("type", '=', 'lead'),('user_id', '=', user_id.id)])
        print("months", months)
        return [
